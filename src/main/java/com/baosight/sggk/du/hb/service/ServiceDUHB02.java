...
public class ServiceDUHB02 extends ServiceEPBase {

	private static final Logger logger = Logger.getLogger(ServiceDUHB02.class);
	...
	public EiInfo initSelectData(EiInfo inInfo) {
		...
		list2.add(map3);
		/*
		 * Map<String, String> map4 = new HashMap<String, String>(); map4.put("typeid",
		 * "P1"); map4.put("typename", "工序"); list2.add(map4);
		 */
		...
		querymap.put("type", "D1");
		List list3 = this.dao.query("tduhb01.query", querymap);
		departblock.addRows(list3);
		inInfo.setBlock(departblock);

		String oprationType = StringUtils.isBlank((String) inInfo.get("oprationType")) ? ""
				: (String) inInfo.get("oprationType");

		if ("insert".equals(oprationType)) {
			String parentid = (String) inInfo.get("parentid");
			parentid = StringUtils.isBlank(parentid) ? "root" : parentid;
	...
	public EiInfo initLoad(EiInfo inInfo) {
		...
		if ("insert".equals(oprationType)) {
			...
			String departmentId = StringUtils.isBlank((String) inInfo.get("departmentId")) ? "空"
					: (String) inInfo.get("departmentId");
			Map querymap = new HashMap();
			querymap.put("departmentId", departmentId);
			List list = this.dao.query("tduhb01.query", querymap);
			if (list.size() > 0) {
				...
				row.put("totalplan", tduhb01.getTotalplan());
			}
		}
	...
	public EiInfo save(EiInfo inInfo) {
		...
		try {
			...
			String totalplan = (String) inInfo.get("inqu_status-0-totalplan");
			...
			if ("insert".equals(oprationType)) {
				...
				sortIsExist(sort, parentid);
				String gDepartment = getDepartmentId();
				map.put("departmentId", gDepartment);
				...
				dao.update("tduhb01.updateDepartInfo", map);
				buffer.insert(0, "记录保存成功\n");
			} else {
	...
	private String getDepartmentId() throws Exception {
		String departmentId = "DEP";
		Map<String, String> pMap = new HashMap<String, String>();
		pMap.put("departmentId", departmentId);
		List list = this.dao.query("tduhb01.queryId", pMap);
		int count = 1;
	...
	private String getMaxSort() {
		String sortVal = "999";
		List<Tduhb01> list = this.dao.query("tduhb01.queryMaxSort", new HashMap<>());
	...
	/**
	 * 新增 根据传递的排序值，去查询是否有对应的排序值存在， 如果存在，将数据表中的id值，依次加1，并将传递的参数排序值加入数据表中，
	 * 如果不存在，则直接插入数据库
	 * 
	 * @param num
	 */
	public void sortIsExist(String sortNum, String parentid) {
		// 1.查询该排序值是否存在
	...
	/**
	 * 修改时的排序方法 根据传递的排序值，去查询是否有对应的排序值存在， 如果存在，将数据表中的id值，依次加1，并将传递的参数排序值加入数据表中，
	 * 如果不存在，则直接插入数据库
	 * 
	 * @param num
	 */
