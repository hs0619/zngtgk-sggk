...
public class ServiceDUHB18 extends ServiceEPBase {

	private static final Logger logger = Logger.getLogger(ServiceDUHB18.class);
...
//		outInfo.setBlock(block);
	...
	public EiInfo queryinfo(EiInfo inInfo) {
		EiInfo outInfo=new EiInfo();
		inInfo.set("inqu_status-0-devicetype", "01");
...
//			"oldresult", "oldresult");
	...
	public EiInfo queryPortInfo(EiInfo inInfo) {
		StringBuffer buffer = new StringBuffer();
		try {
			String portid = StringUtils.isBlank((String) inInfo.get("portid")) ? "空" : (String) inInfo.get("portid");
			Map querymap = new HashMap();
			querymap.put("dischargeportcode", portid);
			List list = this.dao.query("tduhb05.query", querymap);
			if (list.size() > 0) {
				inInfo.set("inqu_status-0-flag", "1");
				inInfo.set("inqu_status-0-dischargeportid", ((Tduhb05) list.get(0)).getDischargeportid());
			}
		} catch (Exception ex) {
			buffer.insert(0, "记录查询失败\n" + ex.toString());
			inInfo.setStatus(-1);
