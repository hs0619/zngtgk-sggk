...
public class ServiceDUHB2102 extends ServiceBase {

	private static final Logger logger = Logger.getLogger(ServiceDUHB2102.class);
	...
	public EiInfo query(EiInfo inInfo) {
		...
		if (StrUtil.listIsNotNullOrEmpty(listhb20)) {
			...
			if (StrUtil.paramIsNotNullOrEmpty(devicecode)) {
				...
				for (int i = 0; i < envdeviceidArr.length; i++) {
					...
					List<Tduhb18> listhb18 = this.dao.query("tduhb18.query",fMap);
		...
		if (StrUtil.listIsNotNullOrEmpty(deviceList)) {
			...
			String outto = tduhb18.getOutto();
			String portouttype = tduhb18.getPortouttype();
			String outrule = tduhb18.getOutrule();
			...
			outInfo.set("inqu_status-0-outto", outto);
			outInfo.set("inqu_status-0-portouttype", portouttype);
			outInfo.set("inqu_status-0-outrule", outrule);
	...
	public EiInfo loadMsg(EiInfo inInfo) {
	    ...
	    if (StrUtil.listIsNotNullOrEmpty(list)) {
			...
			inInfo.set("outto", tduhb18.getOutto());
			inInfo.set("portouttype", tduhb18.getPortouttype());
			inInfo.set("outrule", tduhb18.getOutrule());
