...
public class ServiceDUHB2001 extends ServiceEPBase {

	private static final Logger logger = Logger.getLogger(ServiceDUHB2001.class);
...
//			departblock.addRow(map);
		...
		proceduremap.put("parentid", StringUtils.isBlank(departmentid)?departlist.get(0).getDepartmentId():departmentid);
		...
		if ("insert".equals(oprationType)) {
			...
			row.put("pollutantname", "");
			row.put("facilitytype", "");
			row.put("numbers", "");
			...
			List<Tduhb20> list = this.dao.query("tduhb20.queryForUpdate", querymap);
			if (list.size() > 0) {
				Tduhb20 tduhb20 = list.get(0);
				...
				row.put("pollutantname", tduhb20.getPollutantname());
				row.put("facilitytype", tduhb20.getFacilitytype());
				...
				row.put("handabilityAct", tduhb20.getHandabilityAct());
				row.put("devicearea", tduhb20.getDevicearea());
				row.put("deviceflow", tduhb20.getDeviceflow());
				...
				row.put("craft", tduhb20.getCraft());
				row.put("filter", tduhb20.getFilter());
	...
	public EiInfo query(EiInfo inInfo) {
		...
		inInfo = super.query(inInfo, "tduhb18.query", new Tduhb18());
	...
	public EiInfo save(EiInfo inInfo) {
		...
		try {
			...
			String handabilityAct = (String) inInfo.get("inqu_status-0-handabilityAct");
			String devicearea = (String) inInfo.get("inqu_status-0-devicearea");
			String deviceflow = (String) inInfo.get("inqu_status-0-deviceflow");
			
			String craft = (String) inInfo.get("inqu_status-0-craft");
			String filter = (String) inInfo.get("inqu_status-0-filter");
			...
			map.put("handabilityAct", handabilityAct);
			map.put("devicearea", devicearea);
			map.put("deviceflow", deviceflow);
			map.put("type", "01");
			map.put("status", "1");
			...
			if ("insert".equals(oprationType)) {
				...
				dao.update("tduhb20.updateForFQFacility", map);
				buffer.insert(0, "记录保存成功\n");
			} else {
...
//			String oprationType = (String) inInfo.get("inqu_status-0-oprationType");
//			String facilityid = (String) inInfo.get("inqu_status-0-facilityid");
//			inInfo.getBlock("inqu_status").setCell(0, "type", "01");
...
//				dao.update("tduhb20.updateForFQFacility", inInfo.getBlock("inqu_status").getRow(0));
//			} else {
//				buffer.insert(0, "记录保存失败\n");
//			}
//			buffer.insert(0, "记录保存成功\n");
