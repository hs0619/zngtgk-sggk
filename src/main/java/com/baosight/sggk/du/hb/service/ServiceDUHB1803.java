...
public class ServiceDUHB1803 extends ServiceBase {

	private static final Logger logger = Logger.getLogger(ServiceDUHB1803.class);
	...
	public EiInfo load(EiInfo inInfo) {
		int i = inInfo.getBlock("result").getRowCount();
		String portid = (String) inInfo.getBlock("result").getCell(0, "portid");
		boolean b = portid.contains(",");
		Tduhb05 tduhb05 = null;
		if(b) {
			//tduhb05 = getByDischargeportcode(id[0]);
		}else {
			tduhb05 = getByDischargeportcode(portid);
		}
		if(tduhb05 != null) {
			inInfo.set("inqu_status-0-dischargeportcode", tduhb05.getDischargeportcode());
			String val = tduhb05.getDischargeportname().replace(",","，");
			inInfo.set("inqu_status-0-dischargeportname", val);
			
			inInfo.set("inqu_status-0-isformal", tduhb05.getIsformal());
			System.out.println(tduhb05.getStatus());
			...
			inInfo.set("inqu_status-0-position", tduhb05.getPosition());
			inInfo.set("inqu_status-0-longitude", tduhb05.getLongitude());
			inInfo.set("inqu_status-0-latitude", tduhb05.getLatitude());
			inInfo.set("inqu_status-0-dischargemode", tduhb05.getDischargemode());
			inInfo.set("inqu_status-0-outto", tduhb05.getOutto());
			inInfo.set("inqu_status-0-outtype", tduhb05.getOuttype());
			inInfo.set("inqu_status-0-outrule", tduhb05.getOutrule());
			inInfo.set("inqu_status-0-exhaustheight", tduhb05.getExhaustheight());
			inInfo.set("inqu_status-0-exhaustinside", tduhb05.getExhaustinside());
			inInfo.set("inqu_status-0-importantport", tduhb05.getImportantport());
			...
			inInfo.set("inqu_status-0-executionstandard", tduhb05.getExecutionstandard());
			inInfo.set("inqu_status-0-description", tduhb05.getDescription());
			inInfo.set("inqu_status-0-dischargeportid", tduhb05.getDischargeportid());
			inInfo.set("inqu_status-0-oprationType", "update");
		}
		return inInfo;
	}
	...
	public EiInfo loadMsg(EiInfo inInfo) {
		...
		if(list.size() > 0) {
			...
			inInfo.set("statusDataBase", tduhb05.getStatus());
