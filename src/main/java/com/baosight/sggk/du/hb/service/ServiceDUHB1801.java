...
public class ServiceDUHB1801 extends ServiceEPBase {

	private static final Logger logger = Logger.getLogger(ServiceDUHB1801.class);
	...
	public EiInfo initLoad(EiInfo inInfo) {
        ...
        if(StringUtils.isBlank(oprationType)) {
        	outInfo.setMsg("废气产排污节点参数错误");
			return outInfo;
        }
        outInfo.set("oprationType", oprationType);
        ...
        if("insert".equals(oprationType)) {
    		...
    		if(list.size() > 0) {
    		    ...
    		    row.put("sourcename", tduhb18.getSourcename());
    		    row.put("factorname", tduhb18.getFactorname());
    		    row.put("outtype", tduhb18.getOuttype());
    ...
    public EiInfo save(EiInfo inInfo) {
		...
		try
        {
			...
			String sourcename = (String)inInfo.get("inqu_status-0-sourcename");
			...
			if("insert".equals(oprationType)) {
				...
				map.put("envdeviceid", getEnvDeviceId());
	        	...
	        	dao.update("tduhb18.updateForDevice", map);
	        	buffer.insert(0, "记录保存成功\n");
			}else {
