...
public class ServiceDUHB2003 extends ServiceBase {

	private static final Logger logger = Logger.getLogger(ServiceDUHB2003.class);
	...
	public EiInfo query(EiInfo inInfo) {
		...
		Map<String, String> pMap = new HashMap<>();
		pMap.put("facilityid", facilityid);
		List<Tduhb20> listhb20 = this.dao.query("tduhb20.query",pMap);//废气治理设施
		if (StrUtil.listIsNotNullOrEmpty(listhb20)) {
			String devicecode = listhb20.get(0).getDevicecode();//对应的产污设施id字符串
			if (StrUtil.paramIsNotNullOrEmpty(devicecode)) {
				String[] envdeviceidArr = devicecode.split(",");
				for (int i = 0; i < envdeviceidArr.length; i++) {
					if (StrUtil.listIsNotNullOrEmpty(allList)) {
