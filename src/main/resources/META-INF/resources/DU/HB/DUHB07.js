	...
	IPLATUI.EFGrid = {
			"result" : {
				...
				columns: [
	            	{
	                    ...
	                    template: function(dateItem) {
                        	...
                        	'<input value="编辑" class="k-button k-button-details" type="button" align="center" onclick="showDetail(\''+dateItem.siteid+'\')"/>'+
                        	'<input value="配置信息" class="k-button k-button-configs" type="button" align="center" onclick="showConfig(\''+dateItem.siteid+'\')"/>'+
                        	'</div>';}
	                }
	            ],
				beforeAdd: function (e) {
					...
					var url = IPLATUI.CONTEXT_PATH + "/web/DUHB0701?oprationType=" + oprationType;
...
showDetail = function(siteid){
  	 ...
  	 var url = IPLATUI.CONTEXT_PATH + "/web/DUHB0701?oprationType=" + oprationType + "&siteid=" + siteid;
  	 popWindow.data("kendoWindow").setOptions({
  	        open: function () {
  	        	popWindow.data("kendoWindow").refresh({
  	                url: url
...
showConfig = function(siteid) {
  	 var popWindow = $("#showSF");
  	 var url = IPLATUI.CONTEXT_PATH + "/web/DUHB0702?siteid=" + siteid;
  	 popWindow.data("kendoWindow").setOptions({
  	        open: function () {
  	        	popWindow.data("kendoWindow").refresh({
  	                url: url
