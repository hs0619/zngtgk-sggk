$(function () {
	...
	IPLATUI.EFWindow = {
        "showEnvdevic": {
            close: function (e) {
            	//envdevicGrid.dataSource.page(1);
...
function loadMsg(envdeviceid){
	...
	EiCommunicator.send("DUHB2002", "loadMsg", ei, {
		onSuccess: function(ei) {
...
function removeEnvdevic(){
	...
	if(checkedLength == 0){
		...
		IPLAT.confirm({
            ...
            okFn: function (e) { 
                EiCommunicator.send("DUHB2002", "removeConection", eiInfo, {
                    onSuccess: function (ei) {
                    	if(ei.getStatus() == -1){
	...
	function addInfo(){
		...
		var popWindow = $("#showEnvdevic");
	  	var url = IPLATUI.CONTEXT_PATH + "/web/DUHB2003?facilityid=" + facilityid;
