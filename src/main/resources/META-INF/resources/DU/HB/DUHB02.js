$(function () {
	...
	var initparentid = __eiInfo.get("inqu_status-0-parentid");
	eiInfo.set("oprationType",oprationType);
	eiInfo.set("departmentId",initdepartmentId);
	eiInfo.set("parentid",initparentid);
	
    EiCommunicator.send("DUHB02", "initSelectData", eiInfo, {
        onSuccess: function (ei) {  
        	...
        	if(ei["blocks"]["departList"] != null){
        		var departList = [];
        		departList.push({"valueField": "root", "textField": "长江钢铁"});
    ...
    $("#submitButton").on("click", function (e) {
		...
		IPLAT.confirm({
            message: '<b>确定保存厂部信息吗？</b>',
            okFn: function (e) { 
            	var eiInfo = new EiInfo();
            	eiInfo.setByNode("inqu");
                EiCommunicator.send("DUHB02", "save", eiInfo, {
                    onSuccess: function (ei) {
                    	if(ei.getStatus() == -1){
